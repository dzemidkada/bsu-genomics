...
class FrequentLocationsGenerator(CandidatesGenerator):
    def __init__(self, **kwargs):
        ...
        self._train_data_locations = kwargs.get('train_locations', None)
        self.__preprocess()

    def __preprocess(self):
        assert self._train_data_locations is not None
        available_locations = Counter()
        available_locations.update([
            (row.lat, row.long)
            for _, row in self._train_data_locations.iterrows()
        ])
