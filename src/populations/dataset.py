...
SNP_KEY_COLUMNS = ('s_id', 'db_id', 'birth_region_id', 'ethnographic_group_id', 'appearance_type')
...
class SNPBatch:
    ...
    def __init__(self, path=None):
        ...
        self._variants_df = self._excel_file.parse(self._excel_file.sheet_names[0])
        # Mapping
    ...
    def __variants_preprocess(self):
        ...
        # No duplicates
        assert self._variants_df.shape[0] == self._variants_df.drop_duplicates(self.FULL_VARIANT_COLUMNS).shape[0]
        # No NaNs        
        assert self._variants_df[self.FULL_VARIANT_COLUMNS].isna().sum().sum() == 0
        ...
        self._variants_df[self._full_variants] = self._variants_df[self._full_variants].astype('category')
    ...
    def __survey_preprocess(self):
        self._survey = self._survey[~self._survey.sample_code.isna()].reset_index(drop=True)
...
class SNPDatasetsHandler:
    ...
    def join_datasets(self, new_id, dataset_ids):
        ...
        result_df = result_df[filter_keys(result_df, SNP_KEY_COLUMNS) + variants_set]
